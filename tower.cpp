...
bool checker(vector <int> a, int num)
{
	...
	if (k >= 1)
		...
		cout << "Монаха под номером " << num << " не существует" << endl;
	if (k >= 1)
		return 1;
	else
		return 0;
...
int main()
{
	...
	string path = "C:\\Users\\Ева\\Desktop\\visual studio\\txtmonah.txt";
	...
	f.close();
	//если порядок кратен 4 или равен нулю, то число стоит первым в строке = монах является учителем
	...
	if (n == 1)
	{
		...
		if (k < 1)
			cout << "Такого монаха не существует" << endl;
	}
	if (n == 2)
	{
		vector <int> nu1;
